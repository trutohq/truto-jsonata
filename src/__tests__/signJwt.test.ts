...
describe('signJwt', () => {
	it('should sign a JWT with default options (HS256)', async () => {
		...
		const token = await signJwt({ payload, secretOrPrivateKey: secret })
        console.log(token)
	...
	it('should sign a JWT with custom options (expiresIn, header.kid)', async () => {
		...
		const token = await signJwt({ payload, secretOrPrivateKey: secret, options })
        console.log(token)
		expect(typeof token).toBe('string')
		expect(token.split('.').length).toBe(3)
	})
	...
	it('should throw error for invalid payload', async () => {
		await expect(signJwt({ payload: 'invalid', secretOrPrivateKey: 'secret' })).rejects.toThrow('Payload must be a non-null object')
	})
}) 